...
import { getAllFlags, getFlagByName } from 'dalim-icons'
...
export function FlagDetailed() {
    ...
    // Get icon metadata
    const iconData = getFlagByName(iconName.charAt(0).toUpperCase() + iconName.slice(1))
    ...
    const generateSVG = async (iconSize: number = size[0]): Promise<string> => {
        const Icon = (Flags as Record<string, any>)[iconData.name]
        if (!Icon) return ''
