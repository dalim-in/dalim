...
export function DrawingCanvas({ onPathChange, strokeWidth, strokeColor, savePath, currentPath }: DrawingCanvasProps) {
    ...
    const [draggingPointType, setDraggingPointType] = useState<'point' | 'control1' | 'control2' | null>(null)
    ...
    useEffect(() => {
        ...
        if (points.length > 0) {
            ...
            for (let i = 1; i < points.length; i++) {
                ...
                if (point.controlPoint1 && prevPoint.controlPoint2) {
                    // Cubic bezier curve
                    ctx.bezierCurveTo(prevPoint.controlPoint2.x, prevPoint.controlPoint2.y, point.controlPoint1.x, point.controlPoint1.y, point.x, point.y)
                    ...
                    ctx.quadraticCurveTo(prevPoint.controlPoint2.x, prevPoint.controlPoint2.y, point.x, point.y)
                } else {
                    // Straight line
        ...
        ctx.restore()
    }, [points, pencilPoints, strokeWidth, strokeColor, selectedPointIndex, hoverPointIndex, hoverControlPoint, currentTool, showControlPoints, isDrawing, scale, offset])
    ...
    useEffect(() => {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            if (point.controlPoint1 && prevPoint.controlPoint2) {
                // Cubic bezier curve
                path += ` C ${(prevPoint.controlPoint2.x - offsetX).toFixed(2)} ${(prevPoint.controlPoint2.y - offsetY).toFixed(2)}, ${(point.controlPoint1.x - offsetX).toFixed(2)} ${(point.controlPoint1.y - offsetY).toFixed(2)}, ${(point.x - offsetX).toFixed(2)} ${(point.y - offsetY).toFixed(2)}`
            } else if (point.controlPoint1) {
                // Quadratic bezier curve with only current control point
                path += ` Q ${(point.controlPoint1.x - offsetX).toFixed(2)} ${(point.controlPoint1.y - offsetY).toFixed(2)}, ${(point.x - offsetX).toFixed(2)} ${(point.y - offsetY).toFixed(2)}`
            } else if (prevPoint.controlPoint2) {
                // Quadratic bezier curve with only previous control point
                path += ` Q ${(prevPoint.controlPoint2.x - offsetX).toFixed(2)} ${(prevPoint.controlPoint2.y - offsetY).toFixed(2)}, ${(point.x - offsetX).toFixed(2)} ${(point.y - offsetY).toFixed(2)}`
            } else {
                // Straight line
    ...
    const addCurvaturePoints = (originalPoints: Point[], simplifiedPoints: Point[]): Point[] => {
        ...
        for (let i = 0; i < simplifiedPoints.length - 1; i++) {
            ...
            const startIndex = originalPoints.findIndex((p) => p.x === startPoint.x && p.y === startPoint.y)
            ...
            if (startIndex !== -1 && endIndex !== -1 && endIndex - startIndex > 10) {
                ...
                for (let j = startIndex + 1; j < endIndex; j++) {
                    if (j > 0 && j < originalPoints.length - 1) {
                        const curvature = calculateCurvature(originalPoints[j - 1], originalPoints[j], originalPoints[j + 1])
    ...
    const handleZoom = (e: React.WheelEvent | { deltaY: number; clientX: number; clientY: number }) => {
        const canvas = canvasRef.current
        if (!canvas) return
    ...
    const startDrawing = (e: React.MouseEvent | React.TouchEvent) => {
        ...
        if (currentTool === 'pencil') {
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                if (point.controlPoint1) {
                    const distToControl1 = Math.sqrt((point.controlPoint1.x - x) ** 2 + (point.controlPoint1.y - y) ** 2)
                ...
                if (point.controlPoint2) {
                    const distToControl2 = Math.sqrt((point.controlPoint2.x - x) ** 2 + (point.controlPoint2.y - y) ** 2)
            ...
            if (points.length >= 2) {
                ...
                if (closestDistance < 10 / scale && closestSegmentIndex >= 0) {
                    ...
                    const t = ((newPoint.x - p1.x) * dx + (newPoint.y - p1.y) * dy) / (segmentLength * segmentLength)
                    ...
                    const updatedPoints = [...points.slice(0, closestSegmentIndex + 1), newPoint, ...points.slice(closestSegmentIndex + 1)]
        ...
        <TooltipProvider>
                ...
                <div className="">
                    <div className="flex items-center gap-1">
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <CopyToClipboard
                                    classname="relative top-0 h-8 ml-2  w-fit px-3 flex"
                                    text={currentPath}>
                                    Svg Path
                                </CopyToClipboard>
                            </TooltipTrigger>
                    ...
                    <div className='flex items-center gap-3'>
                        <ToggleGroup
                            type="single"
                            className="my-2 w-60 rounded-xl border p-1"
                            value={currentTool}
                        ...
                        {currentTool === 'pencil' && (
                            <div className="flex justify-between gap-3">
                                ...
                                <div className="grid w-full gap-2">
                                    <Label htmlFor="simplifyTolerance">Point Reduction: {simplifyTolerance.toFixed(1)}</Label>
                                    <Slider
            ...
            <Tooltip>
              ...
              <TooltipContent>Zoom Out (-)</TooltipContent>
                ...
                <div className="bg-main relative h-[85%] overflow-hidden rounded-lg border">
                    ...
                    <canvas
                        ...
                        onWheel={handleZoom}
                        className={`relative z-[2] h-full w-full touch-none ${isPanning ? 'cursor-grab' : currentTool === 'pencil' ? 'cursor-crosshair' : currentTool === 'pen' ? 'cursor-cell' : currentTool === 'add' ? 'cursor-copy' : currentTool === 'remove' ? 'cursor-not-allowed' : 'cursor-default'}`}
                    />
                    <div className="bg-background/80 text-foreground absolute bottom-2 right-2 rounded-md px-2 py-1 font-mono text-xs">{Math.round(scale * 100)}%</div>
                </div>
                ...
                <p className="text-muted-foreground mt-1 text-xs">Zoom: Use mouse wheel, + and - keys, or the zoom buttons. Pan: Hold Space + drag. Reset view: Press 0.</p>
            </div>
        </TooltipProvider>
    )
}
