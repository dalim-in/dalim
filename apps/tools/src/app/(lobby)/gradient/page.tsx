'use client'
import DesktopApp from '@/src/components/tools/gradient/core-ui/desktop-app'
import MobileApp from '@/src/components/tools/gradient/core-ui/mobile-app'
import { useEffect, useState } from 'react'
import { toast } from 'sonner'
import { FONTS } from '@/src/lib/constants'
import { useWallpaperStore } from '@/src/store/wallpaper'
import { useSafariCheck } from '@/src/hooks/use-safari-check'
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        if (!currentFont?.variable) {
            ...
            const closestWeight = availableWeights.reduce((prev, curr) => (Math.abs(curr - store.fontWeight) < Math.abs(prev - store.fontWeight) ? curr : prev))
    ...
    const downloadImage = async () => {
        try {
            const previewCanvas = document.querySelector('#wallpaper canvas') as HTMLCanvasElement
            ...
            ctx.drawImage(previewCanvas, 0, 0, store.resolution.width, store.resolution.height)
            ...
            if (store.sizeMode === 'text') {
                ...
                // Set font properties
                const fontString = `${store.isItalic ? 'italic' : ''} ${store.fontWeight} ${store.fontSize * 16}px ${store.fontFamily}`
                ctx.font = fontString
                ...
                const startY = store.resolution.height / 2 - totalHeight / 2 + store.textPosition.y
            ...
            if (store.sizeMode === 'image' && store.logoImage) {
                ...
                ctx.drawImage(img, store.resolution.width / 2 - width / 2 + store.textPosition.x, store.resolution.height / 2 - height / 2 + store.textPosition.y, width, height)
                ctx.restore()
            }
            ...
            if (isSafari) {
                ...
                link.href = dataUrl
                link.download = `dalim-${store.resolution.width}x${store.resolution.height}.png`
                link.click()
            } else {
    ...
    const handlePaletteChange = () => {
        const generateHarmonious = () => {
            ...
            const lightRange = lightRanges[Math.floor(Math.random() * lightRanges.length)]
